...
func gitTokenSecurityMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		// Check file parameter
		if file := c.Query("file"); file != "" {
			if isGitTokenFile(file) {
		...
		if dir := c.Query("directory"); dir != "" {
			if isGitTokenFile(dir) {
				c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
					"error": "Access to this directory is not permitted.",
				})
				return
			}
		}
...
func getServerFileContents(c *gin.Context) {
	...
	// Check for git token files first
	if isGitTokenFile(p) {
		c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
			"error":      "Access to this file is not permitted.",
...
func getServerListDirectory(c *gin.Context) {
	...
	if stats, err := s.Filesystem().ListDirectory(dir); err != nil {
		...
		filteredStats := make([]filesystem.Stat, 0, len(stats))
		for _, stat := range stats {
			fullPath := filepath.Join(dir, stat.Name())
			if !isGitTokenFile(fullPath) && !isGitTokenFile(stat.Name()) {
...
func putServerRenameFiles(c *gin.Context) {
	...
	// Check for git token files in the operation
	for _, p := range data.Files {
		pf := path.Join(data.Root, p.From)
		pt := path.Join(data.Root, p.To)
		
		if isGitTokenFile(pf) || isGitTokenFile(pt) {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
				"error": "Cannot perform operations on protected system files.",
			})
			return
		}
	}
...
func postServerDeleteFiles(c *gin.Context) {
	...
	// Check for git token files
	for _, p := range data.Files {
		pi := path.Join(data.Root, p)
		if isGitTokenFile(pi) {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
				"error": "Cannot delete protected system files.",
			})
			return
		}
	}
...
func postServerWriteFile(c *gin.Context) {
	...
	if isGitTokenFile(f) {
		c.AbortWithStatusJSON(http.StatusForbidden, gin.H{
			"error": "Cannot write to protected system files.",
		})
		return
	}
